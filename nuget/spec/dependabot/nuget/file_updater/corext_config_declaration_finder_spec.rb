...
require "dependabot/nuget/file_updater/corext_config_declaration_finder"

namespace = Dependabot::Nuget::FileUpdater
RSpec.describe namespace::CorextConfigDeclarationFinder do
  let(:finder) do
    described_class.new(
  ...
  let(:corext_config) do
    Dependabot::DependencyFile.new(
        name: "corext.config",
        content: fixture("corext_configs", fixture_name)
    )
  end
  let(:fixture_name) { "corext.config" }

  describe "#declaration_strings" do
    subject(:declaration_strings) { finder.declaration_strings }

    context "with a basic corext.config file" do
      let(:fixture_name) { "corext.config" }
